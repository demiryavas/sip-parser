...
#undef XX
      ...
      case s_headers_almost_done:
      {
        ...
        if ((parser->flags & F_UPGRADE) &&
           ...
           */
          parser->upgrade =
              (parser->type == SIP_REQUEST || parser->status_code == 101);
        } else {
      ...
      case s_headers_done:
      {
        ...
        if (parser->flags & F_SKIPBODY) {
          ...
          if (parser->type == SIP_REQUEST && !lenient) {
            /* RFC 7230 3.3.3 */
             ...
             */
            SET_ERRNO(SPE_INVALID_TRANSFER_ENCODING);
            RETURN(p - data); /* Error */
          } else {
            /* RFC 7230 3.3.3 */
      ...
      case s_chunk_size_start:
      {
        ...
        if (UNLIKELY(unhex_val == -1)) {
          SET_ERRNO(SPE_INVALID_CHUNK_SIZE);
          goto error;
        }
